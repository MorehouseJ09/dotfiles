...
f=/System/Library/CoreServices/Dock.app/Contents/Resources/DockMenus; sudo cp $f.plist $f-old.plist; sudo defaults write $f finder-running -array-add '<dict><key>command</key><integer>1004</integer><key>name</key><string>Remove From Dock</string></dict>'; sudo chmod 644 $f.plist; osascript -e 'quit app "Dock"'
f=/System/Library/CoreServices/Dock.app/Contents/Resources/DockMenus; sudo cp $f.plist $f-old.plist; sudo defaults write $f trash -array-add '<dict><key>command</key><integer>1004</integer><key>name</key><string>Remove From Dock</string></dict>'; sudo chmod 644 $f.plist; osascript -e 'quit app "Dock"'
